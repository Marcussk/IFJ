...
void suffixes(char* pattern, int suffix[], int max_pattern_len){
	...
	for(i = max_pattern_len -2; i >= 0 ; --i){
		if(  i > pattern_len && suffix[ i + max_pattern_len - 1 -f ]  < i - pattern_len )
			...
			while(pattern_len >= 0 && pattern[ pattern_len ] == pattern[ pattern_len + max_pattern_len - 1 - f ])
				pattern_len--;
...
/* funkcia na zistenie velkosti abecedy vstupneho stringu */
 ...
 int func_find ( char *src, char *search ){
	...
	alfa_len = alfabeth_len( src_len , src );
	...
	while(j < src_len - search_len){
		...
		else if(good_suffix[i] >= bad_character[(int) src[i+j]] - search_len + 1 + i)
			j += good_suffix[i];
		else
